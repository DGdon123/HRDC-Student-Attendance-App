...
import 'package:ym_daa_toce/features/edit_attendance/data/models/attendance_list_model.dart';
import 'package:ym_daa_toce/features/edit_attendance/presentation/views/edit_attendance_section.dart'
    as edit;
import 'package:ym_daa_toce/features/language/language_provider.dart';
...
import '../../../assign_class/presentation/views/attendance_screen.dart';
import '../../../assign_class/presentation/views/attendance_section.dart';
import '../../../drawer/presentation/app_drawer.dart';
import '../../../edit_attendance/presentation/controllers/attendance_list_controller.dart';
import '../../../offline/offline_dashboard/data/models/offline_class_model.dart';
import '../../../offline/offline_dashboard/presentation/controllers/offline_controller.dart';
import '../../../offline/offline_dashboard/presentation/views/oflline_dashboard.dart';
...
import 'attendance_ranking_table.dart';
...
class _DashboardState extends ConsumerState<Dashboard> {
  ...
  Future<void> _loadData() async {
    ...
    prefs.setInt(
      'class_id',
      assignedData!.class_id,
    );
    prefs.setString(
      'class_name',
      assignedData!.class_name,
    );

    prefs.setInt(
      'school_id',
      assignedData!.school_id,
    );
    prefs.setString(
      'section',
      assignedData!.section,
    );
    prefs.setString(
      'section_id',
      assignedData!.section_id,
    );
    prefs.setInt(
      'teacher_id',
     assignedData!.teacher_id,
    );
    prefs.setString(
      'teacher_name',
      assignedData!.teacher_name,
    );
  }
  ...
  void initState() {
    ...
    _connectivitySubscription =
      ...
      setState(() {
        ...
        if (_isConnected) {
          // If online, sync data automatically
  ...
  Future<void> syncData() async {
    ...
    // Do the data syncing here
    await ref.read(offlineControllerProvider.notifier);
    dev.log(offlineControllerProvider.toString());
    setState(() {
  ...
  String formatNepaliTime(DateTime dateTime) {
    ...
    dev.log(nepaliFormattedTime.toString());
  ...
  Widget build(BuildContext context) {
    final popupProvider =
        providers.Provider.of<LanguageProvider>(context, listen: false);
    final pop = popupProvider.dialogAlreadyShown;
    final originalDate = NepaliDateTime.now();
    final currentDate = DateTime.now();
    final dateFormat = DateFormat('yyyy-MM-dd');
    final date2 = dateFormat.format(currentDate);
    // print(originalDate.toString());
    var monthFormatter = NepaliDateFormat.yMMMM().format(
    ...
    var weekdayFormatter = NepaliDateFormat.E().format(originalDate); //
    ...
    assingedClass.when(
      data: (value) {
        ...
        section4 = int.parse(value.section_id);
      },
      error: (error, stackTrace) => Text(error.toString()),
      loading: () => Center(),
    );
    avatarImage = AppImagesConst.kTeacher;
    return Scaffold(
            ...
            Container(
              child: Center(
                child: SwitcherButton(
                  size: 40,
                  onColor: AppColorConst.kappWhiteColor,
                  offColor: CupertinoColors.systemGrey,
                  value: pop,
                  onChange: (value) {
                    final newLocale = _currentLocale.languageCode == 'ne'
                        ? Locale('en', 'US')
                        : Locale('ne', 'NP');
                    ...
                    popupProvider.setDialogAlreadyShown(value);
                  },
                ),
              ),
            ),
                          ...
                          children: [
                            assingedClass.when(
                                data: (value) {
                                  resetCallback:
                                  () {
                                    // Implement the logic to reset the state and data of your Dashboard here
                                    // For example, clear data lists, reset variables, etc.
                                    setState(() {
                                      // Clear or reset your data here
                                      assingedClass = AsyncValue.data(
                                          [] as AssignedClassModel);
                                                  ...
                                                  children: [
                                                    ...
                                                    StreamBuilder<int>(
                                                      ...
                                                      builder:
                                                        ...
                                                        final currentTime =
                                                            DateTime.now();
                                                        final formattedDate = _currentLocale
                                                                    .languageCode ==
                                                                'ne'
                                                            ? formatNepaliTime(
                                                                currentTime)
                                            ...
                                            Center(
                                              child: Padding(
                                                ...
                                                child: ElevatedButton(
                                                  ...
                                                  onPressed: () async {
                                                    if (date?.attendanceDateAd !=
                                                        date2) {
                                                      dev.log(date2.toString());
                                                      ref.invalidate(
                                                              ...
                                                              return GestureDetector(
                                                                ...
                                                                child:
                                                                  ...
                                                                  decoration: BoxDecoration(
                                                                      shape: BoxShape
                                                                          .circle,
                                                                      color: (Theme.of(context).brightness == Brightness.dark ? Colors.white : AppColorConst.kappprimaryColorBlue).withOpacity(_current ==
                                                                              entry.key
                                                                          ? 0.9
                                                                          : 0.4)),
                                                                ),
                                  ...
                                  if (allZeros) {
                                    ...
                                    return Column(
                                      children: [
                                        ...
                                        Center(
                                            child: Text(
                                          "Students Regularity in Pie Chart"
                                              .tr(),
                                          style: TextStyle(
                                              ...
                                              data: (data) {
                                                double maleValue = ((data.males
                                                        ...
                                                        (data.males.map((e) => int.parse(e)).fold(0, (previousValue, element) => previousValue + element) +
                                                            data.females.map((e) => int.parse(e)).fold(
                                                                0,
                                                                (previousValue,
                                                                        element) =>
                                                                    previousValue +
                                  ...
                                  if (allZeros) {
                                    ...
                                    return Column(
                                      children: [
                                        Center(
                                            child: Text(
                                          "Students Regularity in Bar Graph"
                                              .tr(),
                                          style: TextStyle(
                                            fontWeight: FontWeight.w800,
                                            fontSize: 14,
                                            ...
                                            .when(
                                              data: (data) {
                                                ...
                                                for (int i = 0;
                                                    ...
                                                    i++) {
                                                  double y1 = double.parse(
                                                      data.males[i]);
                                                  double y2 = double.parse(
                                                      data.females[i]);
                                                  double y3 = double.parse(
                                                      data.others[i]);
...
class TodayCardOne extends ConsumerWidget {
  ...
  Widget build(BuildContext context, ref) {
    ...
    var studentReasonList = ref.watch(edit.editstudentReasonListProvider);
